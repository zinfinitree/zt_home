...
class ZtApp {     //20200601

    constructor(idx, ZTICNS_ARx, app_zticx, app_codex, TargetNSx, langx, user_namex, AppNSx) {
...
var app_wa = require('./zt_client_app_workarea');
  ...
  //console.log("20240220a start of setValueFromInitialMessageForApp in zt_client_app");
  for (var j = 0; j < msg.queryResponseObjectWA_AR.length; j++) {
  // console.log("20240220b msg.queryResponseObjectWA_AR[j].objectElement_idx_AR.length: " +msg.queryResponseObjectWA_AR[j].objectElement_idx_AR.length);              
...
// //+this.msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode);
      ...
      // start 20230505  store links from root menu option to other menu options
      var new_link_to_menu_option = new app_wa.LinkToMenuOption();
      new_link_to_menu_option.linkToZTIC = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToZTIC;
      new_link_to_menu_option.linkToCode = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode;
      new_link_to_menu_option.status     = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].status;
      new_link_to_menu_option.timestampEff = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].timestampEff;
      new_link_to_menu_option.linkValue    = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkValue;
      new_menu_option.linkToMenuOption_AR.push(new_link_to_menu_option);
      // end 20230505
       
       // add child to root menu option
...
// //+this.msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode)
      ...
      if(msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkTypeCode == 6){   // link type for menu option to menu option
        ...
        //  store links to other menu options
        var new_link_to_menu_option = new app_wa.LinkToMenuOption();
        new_link_to_menu_option.linkToZTIC = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToZTIC;
        new_link_to_menu_option.linkToCode = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode;
        new_link_to_menu_option.status     = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].status;
        new_link_to_menu_option.timestampEff = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].timestampEff;
        new_link_to_menu_option.linkValue    = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkValue;
        var linkToMenuOption_idx = new_menu_option.linkToMenuOption_AR.push(new_link_to_menu_option) -1;
        ...
        this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menu_option_idx].linkToMenuOption_AR[linkToMenuOption_idx].menuOptionIdx = menu_sub_option_idx;
...
// //console.log("Object Element values");
  ...
  //    console.log("20200702 before addMessageOEval...objx.objectKindZTIC, Code: "+objx.objectKindZTIC+" - "+objx.objectKindCode);
    ...
    // check if object element is for app action object namespace
         if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == app_def_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 17){  
          this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menu_option_idx].appAction_AR[app_action_idx].appAction_object_ns = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
    }  // endif  object element for app action object namespace

     // check if object element is for app action object code
     if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == app_def_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 18){  
      this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menu_option_idx].appAction_AR[app_action_idx].appAction_object_code = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
     }  // endif  object element for app action object code
...
if(objx.objectKindZTIC == mmo_ztic && objx.objectKindCode == 1){  // object is a multi media object

  const mmo_str = './zt_client_multimedia_object_generic'; 

  var new_mmo = new (require(mmo_str));

  new_mmo.ztic = objx.objectZTIC;
  new_mmo.code = objx.objectCode;
  new_mmo.id        = objx.id;
  new_mmo.parent_id = objx.parentId;
  var page_idx;
  
  for(var j = 0; j < this.appPage_AR.length; j++){
    if(this.appPage_AR[j].id == new_mmo.parent_id){
       page_idx = j;
    }  // endif

  } // endfor

  var mmo_idx = this.appPage_AR[page_idx].MMO_AR.push(new_mmo) -1;
  ...
  for(var j = 0; j < objx.link_idx_AR.length; j++){
    ...
    if(msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToKindZTIC == base_ztic && msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToKindCode == 39){            // resource object
      var new_mmo_resource =  new mmo_wa.Resource(msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToZTIC, msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode );
        new_mmo_resource.ztic = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToZTIC;
        new_mmo_resource.code = msg.queryResponseLinkWA_AR[objx.link_idx_AR[j]].linkToCode;
     //   new_resource.id       = objx.id;
     //   new_mmo_resource.parent_id = objx.parentId;
        new_mmo_resource.parent_id = objx.id;
  
        const resource_idx = this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR.push(new_mmo_resource) -1;      

    } // endif link to kind is resource object
  } // endfor

}  // endif object is a multi media object 


//console.log("20240908d objx.objectKindZTIC - objx.objectKindCode - base_ztic : "+objx.objectKindZTIC+" - "+objx.objectKindCode+" - "+base_ztic);
if(objx.objectKindZTIC == base_ztic && objx.objectKindCode == 39){  // object is a resource
  //console.log("20240908c object is a resource");
  this.addResourceValues(msg, objx, base_ztic);  
...
html_str = html_str +         "var obj_ns;";                                   // 20230501
...
html_str = html_str +     " openForm();";    // 20230427 test openForm location
...
html_str = html_str   +         "var TargetNS=\"" +this.TargetNS +"\";";   // 20230316
...
html_str = html_str +           " obj_ns   = value_AR[6];";                // 20230501
html_str = html_str +           " obj_ztic = obj_ns;";                     // REVISIT  obj_ns is passed as obj_ztic, rename later
html_str = html_str +           " obj_code = value_AR[7];";                // 20230501
html_str = html_str +           "\n";
// start  menu option action to open new app
html_str = html_str +         "var user_name=\"xx\";";
html_str = html_str +         "var user_password=\"yy\";";
html_str = html_str +         "var  AppNS=\"zinfinitem.com/zin1\";";              // (del) 20230316
...
//                                    NOTE: also add namespace 
html_str = html_str +             " AppNS    = value_AR[6];";   // 20240917
html_str = html_str +             " app_code = value_AR[7];";  // 20240917
var url
var url_app_page_display;
   if(process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"){  // 20250527
       url_app_page_display =  process.env.ZT_CLIENT_IP   + ":" + process.env.ZT_CLIENT_PORT + "/zt/client/app_page_display";
    }
    else 
    {
       url_app_page_display =  process.env.ZT_CLIENT_IP   + "/zt/client/app_page_display";   //  20250527
    }  // endif process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"  // 20250527
   // console.log("20250518 url_app_page_display: "+url_app_page_display);
//html_str = html_str +   "var jqxhr2 = $.post(\"http://localhost:3000/zt/client/app_page_display\",{TargetNS: TargetNS, language: language, user_name: user_name, user_password: user_password, AppNS: AppNS, app_code: app_code, InitPageNS: InitPageNS, init_page_code: init_page_code }, function(data){";  //(del) 20230316
html_str = html_str +   "var jqxhr2 = $.post(\"" + url_app_page_display + "\",{TargetNS: TargetNS, language: language, user_name: user_name, user_password: user_password, AppNS: AppNS, app_code: app_code, InitPageNS: InitPageNS, init_page_code: init_page_code }, function(data){";

html_str = html_str +           "\n"; 
html_str = html_str +           "var id = \"12345\";";
html_str = html_str +           "sessionStorage.setItem(\"sent\", id);"; 
var url_maintain_object_submit;
 if(process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"){  // 20250527
   url_maintain_object_submit =  process.env.ZT_CLIENT_IP + ":"+process.env.ZT_CLIENT_PORT + "/zt/maintain_object_submit";
 }
 else {
   url_maintain_object_submit =  process.env.ZT_CLIENT_IP  + "/zt/maintain_object_submit";           // 20250527
}  // endif process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"  // 20250527
//html_str = html_str +           " var newWindow = window.open(\"http://localhost:3000/zt/maintain_object_submit\", \"_blank\");";  // (del) 20230316
html_str = html_str +           " var newWindow = window.open(\"" + url_maintain_object_submit + "\", \"_blank\");"; 
html_str = html_str +           "\n";
...
html_str = html_str +           "if(maint_mode == \"update\" || maint_mode == \"display\"){";                 //       20230501
html_str = html_str +               " if(obj_ztic == \"\" && obj_code == \"\"){";   // 20230501
html_str = html_str +                  "obj_ztic = document.getElementById('ns_1').value;";    // get value from pop-up
html_str = html_str +                  "obj_code = document.getElementById('code_1').value;";  // get value from pop-up
html_str = html_str +                "}";  // endif obj_ztic == "" && obj_code == ""
...
html_str = html_str +            "}";  // endif maint_mode == update || maint_mode == display
html_str = html_str +           "\n";
...
var url_maintain_object;
if(process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"){  // 20250527
   url_maintain_object =  process.env.ZT_CLIENT_IP + ":" + process.env.ZT_CLIENT_PORT  + "/zt/client/maintain_object";
}
else
{
   url_maintain_object =  process.env.ZT_CLIENT_IP  + "/zt/client/maintain_object";   // 20250527
} // endif process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"  // 20250527


html_str = html_str +           "var jqxhr = $.post(\"" +url_maintain_object + "\",{TargetNS: TargetNS, language: language, status_sel: status_sel, obj_templ_ztic: obj_templ_ztic, obj_templ_code: obj_templ_code, obj_ztic: obj_ztic, obj_code: obj_code, test_AR: test_ARs, maint_mode: maint_mode, maint_format: maint_format }, function(data){"; 

html_str = html_str +         "var id = \"12345\";";
html_str = html_str +        "sessionStorage.setItem(\"sent\", id);";  

var url_maintain_object_submit2;
if(process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"){  // 20250527
   url_maintain_object_submit2 = process.env.ZT_CLIENT_IP + ":" + process.env.ZT_CLIENT_PORT + "/zt/maintain_object_submit"; 
}
else
{
   url_maintain_object_submit2 = process.env.ZT_CLIENT_IP  + "/zt/maintain_object_submit";  // 20250527
} // end if process.env.ZT_CLIENT_IP.toString().trim() == "http://localhost"  // 20250527

html_str = html_str +           " var newWindow = window.open(\"" + url_maintain_object_submit2 + "\", \"_blank\");"; 
...
//html_str = html_str +     " openForm();";    // 20220716a  (del) 20230427 deleted and placed where the form opens all the time
...
var url_default_background_image;
this.appPage_AR.forEach((appPage, page_idx) => {
  this.appPage_AR[page_idx].MMO_AR.forEach((MMO, mmo_idx) => {
    this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR.forEach((resource, resource_idx) => {
        //console.log("20240909b resource path_name_lang_dep: "+this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_lang_dep);
        url_default_background_image = this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_lang_dep;
    });  // end of this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR.forEach((resource, resource_idx) => {  
    });  // end of this.appPage_AR[page_idx].MMO_AR.forEach((appPage, mmo_idx) => {
  });  // end of this.appPage_AR.forEach((appPage, page_idx) => { 


html_str = html_str + "<style>";
html_str = html_str + "body {";
html_str = html_str +  "background-image: url(\'" + url_default_background_image + "\');";
html_str = html_str +  "background-repeat: no-repeat;";
html_str = html_str +  "background-attachment: fixed;"; 
html_str = html_str +  "background-size: 100% 100%;";
html_str = html_str +  "}";
html_str = html_str +  "</style>";
...
//html_str = html_str +       "<button type=\'button\' class=\"btn\" value='" +  value_ARs + "'>Submit</button>"; (del) 20240914
...
//html_str = html_str +		"<h2>ZinfiniTree Agile Development Tracker</h2>";   //(del) 20230410
html_str = html_str +		"<h2>"+ this.app_title + "</h2>";     // 20230410
html_str = html_str +	"</div>";
...
for(var i = 0; i < this.appPage_AR.length; i++){
  for(var j = 0; j < this.appPage_AR[i].menu_AR.length; j++){  
    ...
    this.appPage_AR[i].menu_AR[j].menu_option_AR[k].linkToMenuOption_AR.sort((a, b) => {
 ...
 for(var i = 0; i < this.appPage_AR.length; i++){   
    for(var j = 0; j < this.appPage_AR[i].menu_AR.length; j++){
     ...
     for(var m = 0; m < menuOptionLevel_AR.length; m++){
      //console.log(" 20220626 menuOptionLevel_AR.lengh - m: "+  menuOptionLevel_AR.length + " - "+m);
    ...
    //console.log("  menu_option_label: "+this.appPage_AR[i].menu_AR[j].menu_option_AR[k].menu_option_label);
           if(m != 0){  // by-pass root menu option 20220622
             html_str = html_str + this.getMenuOptionHtml(i,j,k);  
           } // endif
           childFound = false;   // 20230426 test position to reset childFound
           siblingFound = false;  //20230427
...
////          // *** if menu option has unprocessed children
                ...
                if(menuOptionLevel_AR[m].childMenuOption_idx_AR_idx < childMenuOptionIdx_AR_length){  
                 // console.log("%%% 20230426 setting childFound to true based on: "+menuOptionLevel_AR[m].childMenuOption_idx_AR_idx + " - "+childMenuOptionIdx_AR_length );
...
////            console.log("+++ 20210118 siblingDocElem_idx_AR_idx_var: "+siblingDocElem_idx_AR_idx_var);
              ...
              if(!(siblingFound)){
                ...
                html_str = html_str + this.closeMenuSectionHtml(currentLevel, i, j, k, menuOptionLevel_AR);  //i is appPage_idx, j is menu_idx, k is menu option idx
                //menuSectionClosed = true;
...
////          // *** if not child or sibling
               ...
               if(!childFound && !siblingFound){      //    20239424
...
html_str = html_str + this.closeMenuSectionHtml(0);   // 20220620  TEMP DEL 20230423
...
html_str = html_str + "</html>";
//console.log("--20230421 html_str in zt_client_app.js getHtmlForApp(): "+html_str);
return html_str;
...
closeMenuSectionHtml(levelsDownx, appPage_idx, menu_idx, menu_option_idx, menuOptionLevel_AR){

var parent_idxx;
var gp_idx;
var two_levels_up = levelsDownx - 2;
var parent_sibling_cnt;
var processed_parent_sibling_cnt;
var close_parent = false;
if(levelsDownx > 1){
parent_idxx =  this.appPage_AR[appPage_idx].menu_AR[menu_idx].menu_option_AR[menu_option_idx].parent_idx;
gp_idx     =  this.appPage_AR[appPage_idx].menu_AR[menu_idx].menu_option_AR[parent_idxx].parent_idx;  // grand parent idx
parent_sibling_cnt = this.appPage_AR[appPage_idx].menu_AR[menu_idx].menu_option_AR[gp_idx].childMenuOption_idx_AR.length;
processed_parent_sibling_cnt = menuOptionLevel_AR[two_levels_up].childMenuOption_idx_AR_idx + 1;
   if(processed_parent_sibling_cnt > parent_sibling_cnt){close_parent = true;}

} //endif levelsDownx > 1
 

//console.log("running closeMenuSectionHtml for level - close_parent: "+levelsDownx+ " - "+close_parent);
var retHtml_str = "";
  if(levelsDownx == 0){
    retHtml_str = retHtml_str +	  "</ul>";   // 20230423  (del) 20230424
    retHtml_str = retHtml_str +	  "</div>";  // <!-- navbar-collapse.// -->
    retHtml_str = retHtml_str +	 "</div>";   // <!-- container-fluid.// -->
    retHtml_str = retHtml_str +	"</nav>";
    retHtml_str = retHtml_str +	 "</div>";   // <!-- container -->   // added 20230424
  }  // endif
 ...
 if(levelsDownx == 2){
   ...
   //retHtml_str = retHtml_str + "</ul>";  (del) 20220630
    retHtml_str = retHtml_str + "</ul></li>";
    if(close_parent){      // 20230424
      retHtml_str = retHtml_str + "</ul>";
    }
 }
...
var app_action_object_ns;        // 20230501
var app_action_object_code;      // 20230501
var value_AR = [];
...
for(var i = 0; i < this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menuOption_idx].appAction_AR.length; i++){
   ...
   //app_action_format              = this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menuOption_idx].appAction_AR[i].appAction_object_format; (del) 20230502
   app_action_format              = this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menuOption_idx].appAction_AR[i].appAction_format;
   app_action_object_ns    =        this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menuOption_idx].appAction_AR[i].appAction_object_ns;
   app_action_object_code  =        this.appPage_AR[page_idx].menu_AR[menu_idx].menu_option_AR[menuOption_idx].appAction_AR[i].appAction_object_code;
} // endfor
...
value_AR.push(app_action_object_ns);
value_AR.push(app_action_object_code);
var value_ARs = JSON.stringify(value_AR);

//console.log("&&& 20230502 maint_mode - maint_format: "+ app_action_maint_mode + " - " + app_action_format);
//menu_option_label = menu_option_label + "lvls dwn: "+levelsDown.toString().trim();  // 20230423  TEMP TO BE REMOVED
...
addResourceValues(msg, objx, base_ztic){
  ...
  this.appPage_AR.forEach((appPage, page_idx) => {
    this.appPage_AR[page_idx].MMO_AR.forEach((MMO, mmo_idx) => {
      this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR.forEach((resource, resource_idx) => {
        //console.log("20240909e resource.ztic - resource.code - objx.objectZTIC - objx.objectCode: "+resource.ztic+" - "+resource.code+" - "+ objx.objectZTIC+" - " + objx.objectCode);
        if(resource.ztic == objx.objectZTIC && resource.code == objx.objectCode){

   // //console.log("Object Element values");
   //console.log("20240909d objx.objectElement_idx_AR.length: "+objx.objectElement_idx_AR.length);
   for(var j = 0; j < objx.objectElement_idx_AR.length; j++){
    //    console.log("20200702 before addMessageOEval...objx.objectKindZTIC, Code: "+objx.objectKindZTIC+" - "+objx.objectKindCode);
     //  console.log("20240909c objectElementValue: "+msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue)
       // check if object element is Resource Path/Name
        if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == base_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 210){  
            this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
  
       } // endif object element is Resource Path/Name

      // check if object element is Resource Path/Name--Patch Level Dependent
           if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == base_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 211){  
                this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_patch_dep = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
      
       } // endif object element is Resource Path/Name--Patch Level Dependent
  
       // check if object element is Resource Path/Name--Language Dependent
       if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == base_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 212){  
            this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_lang_dep = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
  
       } // endif object element is Resource Path/Name--Language Dependent

      // check if object element is Resource Path/Name--Patch Level and Language Dependent
        if(msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC == base_ztic && msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode == 213){  
                this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_path_and_lang_dep = msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue;
      
        } // endif object element is Resource Path/Name--Patch Level and Language Dependent
           // console.log("20200604 msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue: "+msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue);
           // console.log("   msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC: "+msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementZTIC);
           // console.log("   msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode: "+msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementCode);
  ...
  // //+this.msg.queryResponseObjectElementWA_AR[objx.objectElement_idx_AR[j]].objectElementValue);
      ...
      }  // endif resource.ztic == objx.objectZTIC && resource.code == objx.objectCode
        ...
        //console.log("20240909a list resource values")
        this.appPage_AR.forEach((appPage, page_idx) => {
          this.appPage_AR[page_idx].MMO_AR.forEach((MMO, mmo_idx) => {
            this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR.forEach((resource, resource_idx) => {
             //   console.log("20240909b resource path_name_lang_dep: "+this.appPage_AR[page_idx].MMO_AR[mmo_idx].resource_AR[resource_idx].path_name_lang_dep);
